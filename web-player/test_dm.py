...
def createModel(characterdict, currentSession, mylanguage, test_par):
    ...
    if test_par.automatic_par == False:
        ...
        try:
            f = open('static/scripts/all-characters-automatic.json', 'r', encoding='utf-8')
        except IOError:
            print("Error: File does not appear to exist.")
    ...
    for i in range(0, len(resp["rows"]) - 1, 1):
        ...
        if (mylanguage == "English"):
            ...
            for i in range(totalUnigrams):
                ...
                if test_par.bigram == True:
                    if i < totalUnigrams - 1:
                        ...
                        bigram_stem = porterStemmer.stem(unigram_list[i]) + "_" + porterStemmer.stem(
                            unigram_list[i + 1])
            ...
            unigram_split = question.strip('،"!؟/)').replace("،", " ").replace("/", " ").split() + answer.strip(
            ...
            for i in range(totalUnigrams):
                ...
                if test_par.bigram == True:
                    if (i < totalUnigrams - 1):
                        ...
                        bigram_stem = \
                        StarMorphModules.analyze_word(unigram_list[i], False)[0].split()[1].replace("stem:", "").split('d',
                                                                                                                       1)[
                            0] + "_" + \
                        StarMorphModules.analyze_word(unigram_list[i + 1], False)[0].split()[1].replace("stem:", "").split(
                            'd', 1)[0]
                        ...
                        bigram_lemma = \
                        StarMorphModules.analyze_word(unigram_list[i], False)[0].split()[0].replace("lex:", "").split('_',
                                                                                                                      1)[
                            0] + "_" + \
                        StarMorphModules.analyze_word(unigram_list[i + 1], False)[0].split()[0].replace("lex:", "").split(
                            '_', 1)[0]
                ...
                if test_par.trigram == True:
                    if i < totalUnigrams - 2:
                        ...
                        StarMorphModules.analyze_word(unigram_list[i + 1], False)[0].split()[1].replace("stem:", "").split(
                            'd', 1)[0] + "_" + \
                        StarMorphModules.analyze_word(unigram_list[i + 2], False)[0].split()[1].replace("stem:", "").split(
                            'd', 1)[0]
                        if trigram_stem not in characterdict[character].stemmedMap.keys():
                            characterdict[character].stemmedMap[trigram_stem] = {}
                        if ID not in characterdict[character].stemmedMap[trigram_stem]:
                        ...
                        StarMorphModules.analyze_word(unigram_list[i + 1], False)[0].split()[0].replace("lex:", "").split(
                            '_', 1)[0] + "_" + \
                        StarMorphModules.analyze_word(unigram_list[i + 2], False)[0].split()[0].replace("lex:", "").split(
                            '_', 1)[0]
                        if trigram_lemma not in characterdict[character].lemmatizedMap.keys():
                            characterdict[character].lemmatizedMap[trigram_lemma] = {}
                        if ID not in characterdict[character].lemmatizedMap[trigram_lemma]:
            ...
            if test_par.unigram == True:
                for token in (unigram_list + unigram_synonyms_list):
                    # print(token)
                    stem = StarMorphModules.analyze_word(token, False)[0].split()[1].replace("stem:", "").split('d', 1)[
                        0]
                    lemma = StarMorphModules.analyze_word(token, False)[0].split()[0].replace("lex:", "").split('_', 1)[
                        0]
    ...
    calculateTFIDF(characterdict, test_par)
...
def direct_intersection_match_Arabic(query, characterModel, test_par):
    queryList = [tmp.strip('،!؟."') for tmp in query.split()]
...
def lemma_intersection_match_Arabic(query, characterModel, test_par):
...
def calculateTFIDF(characterdict, test_par):
        ...
        for stem in characterdict[avatar].stemmedMap.keys():
            ...
            for doc in characterdict[avatar].stemmedMap[stem].keys():
                if test_par.tfidf == True:
                    ...
                    characterdict[avatar].stemmedMap[stem][doc] = 1
        ...
        for word in characterdict[avatar].wordMap.keys():
            ...
            for doc in characterdict[avatar].wordMap[word].keys():
                if test_par.tfidf == True:
                    ...
                    characterdict[avatar].wordMap[word][doc] = 1
...
def findResponse(query, characterModel, currentSession, test_par):
    ...
    if language == "English":
        ...
        stem_match__responses = stem_intersection_match_Arabic(query, characterModel, test_par)
