...
def my_form_post():
    ...
    processed_text = text
    dialogue_manager3.readJsonFile(characterModel)

    global currentAvatar
    tmpAvatar = dialogue_manager3.determineAvatar(processed_text, currentAvatar)
    currentAvatar = tmpAvatar

    response = dialogue_manager3.findResponse(processed_text, characterModel[currentAvatar])
